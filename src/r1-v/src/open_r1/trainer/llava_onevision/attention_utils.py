...
def add_heat_to_image(score_maps, base_path, temporal_patch=2):
    ...
    for i in range(temporal_patch):
        ...
        images = [plt.imread(image_path)[..., :3]*255 for image_path in image_paths]
    ...
    image_heat = np.concatenate(image_heap_2_norm+image_heap_2+image_heap_2_ori, axis=1).astype(np.uint8)
    return image_heat
...
class Qwen2Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        #     attn_weights = torch.matmul(query_states.float(), key_states.transpose(2, 3).float()) * self.scaling
        #     attn_weights = attn_weights.softmax(dim=-1)
        #     attn_weights = attn_weights.mean(dim=(1, 2))
        #     score_for_save = attn_weights[video_mask].reshape(*video_grid_thw)
        #     image_heat = add_heat_to_image(score_for_save.detach().cpu().numpy(), debug_path, temporal_patch=1)
        #     plt.imsave(f"{debug_path}/image_heat_attn_{self.layer_idx}.png", image_heat)
        ###################
