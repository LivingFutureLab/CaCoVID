...
class LlavaOnevisionCausalLMOutputWithPast(ModelOutput):
    ...
    frame_selection_logits: Optional[Tuple[torch.FloatTensor]] = None
...
class LlavaOnevisionForConditionalGeneration(LlavaOnevisionPreTrainedModel, GenerationMixin):
    def __init__(self, config: LlavaOnevisionConfig, **kwargs):
        ...
        self.token_select_strategy = getattr(config.vision_config, "token_select_strategy", False)
    ...
    def get_image_features(
        ...
        image_features = self.vision_tower(pixel_values, output_hidden_states=True, output_attentions=True)
    ...
    def get_video_features(
        ...
        video_features = self.vision_tower(pixel_values, output_hidden_states=True, output_attentions=True)
        extra_info = video_features
        ...
        return video_features, extra_info
    ...
    def generate(
        ...
        old_frame_policy_logits: Optional[torch.Tensor] = None,
        old_token_policy_logits: Optional[torch.Tensor] = None,
        **kwargs,
    ):
        r"""
        ...
        self.old_frame_policy_logits = old_frame_policy_logits
        self.old_token_policy_logits = old_token_policy_logits
        ...
        if return_vision_info:
            vision_info = {
                ...
                "frame_selection_mask": getattr(self, "frame_selection_mask", None),
            }
    ...
    def forward(
        ...
        **kwargs,
        ...
        if pixel_values_videos is not None:
            ...
            video_features, vision_extra_info = self.get_video_features(
                pixel_values_videos,
                vision_feature_layer=vision_feature_layer,
                vision_feature_select_strategy=vision_feature_select_strategy,
            )
            ...
            if self.apply_compression:
                if video_mask_for_refer_model is None:
                    self.video_selection_mask, extra_info = self.compression_policy(inputs_embeds=inputs_embeds, 
                                            ...
                                            video_embeds=None,
                                            token_select_strategy=self.token_select_strategy,
                                            video_grid_thw=self.video_grid_thw,
                                            topk=self.topk,
                                            max_sample_rate=max_sample_rate,
                                            doc_id=getattr(self, "doc_id", None),
                                            old_token_policy_logits=self.old_token_policy_logits,
                                            old_frame_policy_logits=self.old_frame_policy_logits)
                    self.frame_selection_mask = extra_info.get("frame_compress_sample", None)
        ...
        if os.environ.get('DEBUG', "0") == '3':
            self.video_selection_mask = torch.ones_like(self.video_selection_mask).long()
            extra_info = {}
        ...
        if self.apply_compression:
            if video_mask_for_refer_model is not None:
                ...
                return LlavaOnevisionCausalLMOutputWithPast(
                    ...
                    frame_selection_logits = extra_info.get("frame_compress_logits", None),
        ...
        return LlavaOnevisionCausalLMOutputWithPast(
            ...
            video_selection_logits = video_selection_logits if video_selection_mask_for_refer_model is not None else None,
            frame_selection_logits = extra_info.get("frame_compress_logits", None) if video_selection_mask_for_refer_model is not None else None,
